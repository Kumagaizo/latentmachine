...
The hosted endpoint at `https://www.latentmachine.com/api/mcp` is different: payloads travel to Latentmachine's Vercel function for stateless processing. Its contract run/check tools default to privacy-safe report shaping, but that does not make the network transfer local.
