...
`result.clusters` reports up to three coherent rules with a stable result-local `label`, a privacy-safe source-to-target `signature`, support, and batch share. Signatures describe paths, never row values. When one cluster has majority support, rows outside it are flagged after full-batch replay. When coherent clusters split the batch without a majority, the verdict is `unverifiable` and no individual row is accused. `result.unexplained` contains the bounded list of row indices not covered by those clusters.
...
Signed numeric inference supports magnitude fields through an absolute-value `numericTransform` with an optional divisor, plus `numericCompare` flags for values below or above zero. Both require examples on both sides of the comparison so a one-sided batch cannot manufacture sign semantics. Exact multiplication of two row fields continues to use `numericBinary`; rounded or scaled two-field products remain unverifiable unless another supported symbolic rule explains them.
